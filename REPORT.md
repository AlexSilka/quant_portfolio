...
canonical portfolio** (`scripts/run_master_book.py`). The deliverable is a **four-family master book**:

> **Four families are traded** — short-vol / VRP, cross-sectional momentum, trend and breakout. A further set clears the same
> validation and is **not held** (carry, global-macro, crisis-alpha, betting-against-beta and residual momentum): what that composition costs, and the whole
> search behind it, is in **§6d-ter**.
> That choice is the only one here made against the scorecard rather than before it, and every candidate in the
> search is assembled by the same code that assembles the book.
> The four are combined at **genuine equal-weight risk parity** (no fitted weights) on each family's honest,
> **survivorship-free / point-in-time** series over a **15-year window (2012 → 2026)** — the short-vol leg
...
> The book nets **Sharpe 3.94** at **−9.4% max drawdown** — on the brief's **$500k** sizing
> capital that is **$3.45M** of P&L, **~$237k/yr** — **+47.4%/yr** not reinvested,
> **+59.4%/yr** compounded (a rate, not a reachable balance: capacity caps the book long before the end of
> the window) — months-in-profit **78%**, **positive in all 15 calendar years**.
> **§11 scores the targets on the frozen out-of-sample block**, and there it clears **5/5** (2024-07→: Sharpe
> **3.80**, months **84.6%**, max-DD **−3.9%**, worst month **−1.7%**, streak **1**).
> On the **full 15-year window** it also clears **2/5** (Sharpe **3.94**, months **78.4%**, max-DD
> **−9.4%**, worst month **−6.4%**, streak **3**). What that changed, measured
> in §6d-ter: **return went up**, not down — +30.0pp of CAGR on the full window,
> +18.5pp on the block — because six legs at equal risk run hotter than eight; the Sharpe reads
> **+1.50** on the block for the same reason, and at matched risk the wider book edges it by
> −25.7pp. **The real price is concentration and breadth**: the short-vol leg's share of P&L up
> from **67% to 79%**, and **no family left that spans both asset classes**.
...
The book is a **volprem-anchored, diversified** four-family portfolio. Short-vol / VRP carries the Sharpe
(4.78 standalone with the gate, and the gate is what makes that number ownable: the
ungated leg's worst day is −75.4% on a −77.0% drawdown, the gated one's is
−13.6% on −22.8%); the other three families
(standalone 0.0–1.0, mean pairwise correlation ≈ 0.06) **cut that tail and make the book survivable** — so as
they join, the marginal curve *falls* from volprem's 4.78 toward the combined 4.02 while the shipped
book's worst month is **−6.4%** and max drawdown **−9.4%** — the VIX regime gate flattens the short-vol
tail that used to set the deep months. Remove the anchor (volprem) and a genuine **Sharpe +1.03** book still
stands — decorrelated, positive every year — so it is not one premium alone; the diversifiers buy robustness,
not headline Sharpe. (volprem is 82% of book P&L, so this concentration is itself a stated risk, not a
hidden one.)
...
- **Honest universes, honest levels — now including trend.** Every family uses its **survivorship-free**
  ...
  are biased. Levels are quoted on the **15-year window (2012 → 2026)**; each family joins as it lists,
  averaged over the families live each day. The pre-2020 window runs the long-history legs (short-vol / VRP and trend)
  on **real, liquid ETF / FX / index prices** (SPY / GLD / TLT / EM-FX, back to 2011 — the standard managed-futures
  backtest); cross-sectional momentum and breakout list from 2020. **The headline is window-robust** — the same book scored on
  different reporting windows nets Sharpe **3.99 full-history / 3.99 15-year / 3.69 10-year**, so nothing hinges
  on the early window.
- **Robust, not fitted.** The portfolio is robust because the families are decorrelated — measured
  (block-bootstrap MC-P5 **+3.31**), not asserted — and **positive in 15 of 15 calendar years** 2011–26 (weakest 2022 at +0.5).
  Against the task scorecard, the book scores **5/5 out-of-sample and 2/5 on the full window** — the final out-of-sample block
  (2024-07→, the window the brief scores: Sharpe **3.80**, months-in-profit **84.6%**, max-DD −3.9%, worst month
  −1.7%, streak 1) and the **full 15-year window** (Sharpe **3.94**, months-in-profit **78.4%**, max-DD −9.4%,
  worst month −6.4%, streak 3). Months-in-profit ≥80%, the worst month and the ≤2-month streak hold
...
- **Where the margin is thin — stated, not buried.** The binding target is no longer the drawdown but the
  **worst month**: at **−6.4%** against **−6%** it fails both accounting conventions
  (fixed-$500k reads **−6.39%**), but it is a single month (Aug-2015; the next worst is
  −6.0%) sitting close to the floor, and the bootstrap puts a −9.2% month inside its
  5th percentile.
  **1.15× is the level that ships, on both conventions** — 1.20× fails both, and
  1.25× fails both (§4b). **The −78% tail belongs to the UNGATED leg, and the shipped one does not carry it** — a sentence here
  used to say the opposite, and the artifact it was describing disagreed. Measured on the published
  series: ungated worst day −75.4% / drawdown −77.0% / skew
  −18.4, gated −13.6% / −22.8% / −3.3. Through the
  ...
  with both curve segments live (2011+) the leg's worst day is −11.2%, and it
...
- **Discovery + twelve family deep-dives** — the search layer (`run_book.py`) tests every
  ...
  **2,129 candidate sleeves generated → 105 pass in-sample (Sharpe > 0.5) → 60 pass walk-forward (their own
...
- **Portfolio assembly** — one canonical script (`scripts/run_master_book.py`) risk-parity-combines the
  four traded families from their published honest series into the master book (§4); the two
...
- **Backtest** — bar-close→execution delay (no same-bar fill), liquidity-aware costs (commission +
  ...
  the book re-charged at **1×/2×/3×** the rebalancing cost it is charged nets Sharpe **+3.94 /
  +3.90 / +3.87** (max-DD −9.4% / −9.6% / −9.8%),
  **break-even at ≈beyond the highest multiple tested — costs never take it negative**; that charge is deliberately conservative — it counts the mixed 252/365
  calendar's weekend renormalisation as trading, so it bills ~8× round-trip a year against the
  ~nan× the book actually rebalances. **Per family, cost as a share of gross P&L**, each measured
...
|---|---|---|---|
| crisis-alpha | 63.1% | 1.6× | **yes** |
| carry | 23.2% | 4.3× | no |
| breakout | 19.6% | 5.1× | no |
| x-sect | 14.3% | 7.0× | no |
| global-macro | 13.5% | 7.4× | no |
| BAB | 4.7% | 21.4× | no |

  \* carry's re-run reproduces the published series to 2.6% rather than exactly, so its share is
  approximate. **1 of the six measured is cost-fragile: crisis-alpha**, which
  pays 63.1% of its gross P&L in cost and breaks even at 1.6× — expected for a
  ...
  break-even (vol-prem 0.85 at 5×; trend 0.87 at 3×); neither is fragile.
- **Sizing capital and what the dollar figures mean (§9).** The brief fixes **$500k of capital for sizing and
  ...
  quoted at that size with **P&L not reinvested**: **$3.45M** over the 15-year window, **~$237k/yr**, worst month
  **−$32,458**, deepest drawdown **−$48,539**. **The accounting convention no longer moves any target** — the
  scorecard is compounded (risk a constant fraction of capital, max-DD −9.39%, worst month −6.36%), while holding
  size fixed at $500k and taking percentages *of that same capital* gives **−9.71%** and **−6.49%**: ~0.2pp
  stricter, and the verdict is the same **2 of 5 either way** — the miss is months-in-profit under 80% and a 3-month losing streak against ≤2 and a worst month past −6%, which no
...
re-scales each to ~15% vol on trailing (lagged) vol, and **equal-weights the 4 earners (1/N — genuine risk
...
curve is inverted or the S&P is 12% off its trailing-year high, averaging ~0.70
...
per-family weight cap (1.5× the 1/4 equal weight; at equal weight it never binds). The drawdown ladder is ~neutral on this benign-tail
history (dormant insurance); the **VIX gate is the active risk layer** — it times the short-vol leg out of the crashes that
cluster the losing months, holding the book at **Sharpe 3.94** and closing the scorecard to
**5/5 out-of-sample, 2/5 on the full window** (§5d/§6). 15-year window (2012 → 2026); each family joins as it lists, averaged over those live each day. **Mean
pairwise cross-family correlation is ≈ 0.06** — the corr-to-book column is naturally higher since each
family is part of the book. **The decorrelation is stable out-of-sample** — the same matrix re-measured on two halves of the
window and on the frozen block reads first-half 0.05 / second-half 0.07 / OOS-block 0.01, max pairwise shift 0.05 — not an
in-sample artifact.

| family | honest series | standalone Sharpe | corr to book |
|---|---|---|---|
| **vol-premium** | short-vol / VRP across 18 Cboe underlyings (incl. gold-miners), 2005+ ([docs/strategies/VOLPREM.md](docs/strategies/VOLPREM.md)) | 4.78 | +0.72 |
| **breakout** | crypto trend+ML / PIT top-30 x-sect ([docs/strategies/BREAKOUT.md](docs/strategies/BREAKOUT.md)) | 0.92 | +0.54 |
| **trend_momentum** |  | 0.87 | +0.58 |
| **x-sect momentum** | crypto residual (idio) + equity, top-100 liquid ([docs/strategies/XSECT.md](docs/strategies/XSECT.md)) | 0.83 | +0.56 |
...
> raw figure — e.g. vol-premium **4.78** here (gated) vs **+3.58** raw, ungated
> ([VOLPREM.md](docs/strategies/VOLPREM.md)); carry **1.16** vs **+1.21** raw ([CARRY.md](docs/strategies/CARRY.md)) —
...
- **Master book (risk-managed deliverable, 1.15× = ~12.8% book vol):** full-sample Sharpe **3.94**; on the brief's
  **$500k** sizing capital **$3.45M** of P&L, **~$237k/yr** — **+47.4%/yr**
  not reinvested, **+59.4%/yr** compounded (§9) — max DD **−9.4%**, months-in-profit
  **78.4%**, worst month **−6.4%**, streak **3mo** — **2 of 5 on the
  15-year window** (months-in-profit under 80% and a 3-month losing streak against ≤2 and a worst month past −6%);
  block-bootstrap MC **[Sharpe P5 +3.31, P50 +3.96, P95 +4.62; max-DD P5 −16.2%, P50 −11.2%]**; mean
  pairwise cross-family correlation **+0.06**. **On the final OOS block: Sharpe 3.80, months-in-profit
  84.6%, max-DD −3.9%, worst −1.7%, streak 1mo — 5/5.**
  Per-family P&L share: **volprem 82%**, trend 12%, x-sect 3%, breakout 3% — volprem-dominated, stated not hidden.
...
- **Marginal contribution** (standalone-descending, on the premium stack, short-vol leg VIX-gated), families
  added in that order: **4.78 → 4.58 → 4.12 → 4.02** — the curve *falls* as diversifiers join: they trade a little
  Sharpe for a much smaller tail (−17.0% at the anchor alone, and along the last four additions
  −17.0% → −17.0% → −9.9% → −9.4%). Removing the anchor (vol-premium) still leaves **+1.03**. `master_book_marginal.csv` carries
...
- **What happens when two legs want the same capital, or opposite sides of the same asset.** Capital is
  ...
  1/4 of book risk, so a loud signal in one family cannot crowd out another — it can only use its own
...
- **Per-year Sharpe (regime profile):** **positive in all 15 calendar years** 2012–2026 —
  2012 **+4.4** · 2013 **+7.3** · 2014 **+3.3** · 2015 **+3.3** · 2016 **+4.8** · 2017 **+7.8** · 2018 **+1.1** · 2019 **+3.5** · 2020 **+2.5** · 2021 **+5.2** · 2022 **+0.5** · 2023 **+3.9** · 2024 **+2.5** · 2025 **+4.9** · 2026 **+2.2** (weakest 2022 at +0.5, a partial year). No down *year*, but through the **isolated crisis windows the book is negative**
  (Q1-2016 is the deepest quarter and contains the −6.4% worst month, Aug-2015;
...
The equal-weight stack runs at **10.7% annualised volatility** on its own, against a **−15% drawdown mandate**, so
...
> (vol-premium and x-sect sit on the cap at most 0.0% of days). §Operational reality below is where this can and
> cannot be filled.
...
- **The 2010 event** — the vol-premium leg's documented systemic tail, replayed inside the four-leg book. It is
  real and dated: **2010-05-06, the flash crash, −76.4% on the leg in a single day** (not evaluated (no family in this composition has 2010 history) at its book weight),
  ...
  (crisis **+2.0%**, global-macro **+4.1%** that day) — and the event is spliced into each of **no quarters** of the
  current book, reported at its **worst** placement. Unlevered it costs the book **not evaluated (no family in this composition has 2010 history) in one day**;
...
|---|---|---|---|---|---|---|---|---|---|---|
| 1.00× | 4.00 | +51.5% | −8.4% | −5.4% | 79.0% | 3/5 | −13.8% | −8.1% | — | — |
| **1.15× (shipped)** | **3.99** | **+60.5%** | **−9.2%** | **−6.3%** | **78.4%** | **2/5** | **−15.7%** | **−8.9%** | **—** | **—** |
| 1.20× | 3.99 | +63.4% | −9.6% | −6.5% | 78.4% | 2/5 | −16.3% | −9.3% | — | — |
| 1.25× | 3.93 | +65.2% | −10.4% | −6.8% | 78.4% | 2/5 | −17.5% | −9.7% | — | — |
| 1.35× | 3.91 | +70.9% | −12.3% | −6.8% | 76.1% | 2/5 | −19.3% | −10.3% | — | — |
| 1.45× | 3.92 | +77.1% | −13.4% | −6.8% | 75.6% | 2/5 | −20.1% | −10.9% | — | — |
| 1.65× | 3.85 | +88.4% | −15.9% | −8.2% | 73.9% | 1/5 | −23.0% | −12.2% | — | — |
| 2.00× | 3.80 | +108.7% | −22.5% | −10.3% | 71.6% | 1/5 | −27.2% | −14.5% | — | — |

**Sharpe is flat in leverage** (4.01 → 3.80 across the whole grid — the small decay is the §8 overlay, not the
...
−6.5% against the −6% floor, while the realised max-DD never breaks across the whole grid
(above ~1.5× the ladder starts cutting and caps it) and the full window's losing streak is
**3–6 at every rung**. The exact ceilings:

| constraint | largest leverage that still holds |
|---|---|
| realised max-DD | 1.55× |
| realised worst month | **1.10×** |
| realised months-in-profit | fails already at 1.00× |
| bootstrap-P5 max-DD | **1.10×** |
| bootstrap-P5 worst month | fails already at 1.00× |
| 2010-event max-DD | fails already at 1.00× |
...
**[3.81, 3.86]** and the streak **1** all the way, and only the risk metrics
scale — max-DD **−3.4% → −6.6%**, worst month **−1.5% → −3.3%** from 1.00× to 2.00×
(CAGR 33.0% → 73.7%). That block is a calm two years; it is shown, not used.

**The chosen level: 1.15×** (`BOOK_LEVERAGE`, ≈12.8% realised book vol), and on the
four-family book it sits **exactly on** its binding realised constraint rather than inside it: the
realised worst month reads −6.4% here and −6.5% one rung up, so
1.10× is the last level that clears the −6% floor. The tail readings bind harder still — the
**realised worst month** allows only **1.10×**, and both tail worst-month readings fail at 1.00×.
...
book the scorecard and the mandate agree on the same rung: 1.10× is both the last level whose
...
window nobody reports on, because only **one** of the four legs exist there
(vol-premium; the crypto legs have no 2005) — and the book fails the targets at **any** leverage: at 1.00× it
already runs months-in-profit **77.3%** with a **3-month** losing streak
and a **−5.3%** worst month, on a **−9.1%** drawdown that is already outside
...
unlucky path of its *own* return distribution the book's worst month is **−9.2%**, and a repeat of the
2010 event costs **+nan%**. The realised worst month is not the cushion it looks like either — it reads
−6.4% against the −6% floor at the shipped level, but every extra 0.1× makes the *modelled* tail
proportionally worse while the realised one improves only on paper. Only the drawdown dimension genuinely has
room (−9.4% against −15%). The honest summary: the book is under-risked **on drawdown** and constrained by
...
short-vol leg (which carries the tail *and* 82% of P&L) at 1.00× and give the leverage to the other
three. Judged
**vol-matched** — same book risk, since any sizing change that raises risk raises return — it loses:

| | all four legs 1.15× | three legs 1.32×, vol-prem 1.00× |
|---|---|---|
| Sharpe (full / OOS) | **3.99 / 3.86** | 3.64 / 3.42 |
| CAGR | **+60.5%** | +53.7% |
| max-DD | **−9.2%** | −11.1% |
| worst month | **−6.3%** | −6.8% |
| months-in-profit | **78.4%** | 77.3% |
| targets, full window | **2/5** | 2/5 |
| 2010-event DD / month | +nan% / +nan% | +nan% / +nan% |

The mechanism does work in the direction intended — the event's worst month softens from +nan%
to +nan%. But volprem is not merely the aggressive leg, it is also the highest-Sharpe and most
decorrelated one, so shifting weight to the other three (which are more correlated *with each
other* than with it) buys less risk reduction than it costs in return: −0.35 Sharpe,
−6.8pp CAGR, and 1.1pp of months-in-profit, which **breaks the ≥80% target and
drops the book to 2/5**. Pushing further — de-levering
...
- At the shipped 1.15× the two conventions differ where it now counts (max-DD −9.39% compounded vs
  −9.71% on the fixed $500k, worst month **−6.36% vs −6.49%**): the
...
them sets book risk: the per-leg target decides how the four legs are balanced *against each
other*, and it is the wrong dial for sizing the book, because it does not scale. Raise it by the same 15% and the
two books differ on **2422 of 4,380 days** — the days `_scale`'s 3× cap binds, which is where the legs are most
levered and the book's worst days live. The result is a *quieter* book at the same nominal size
(−9.9% max-DD against −9.4%, worst month −6.2% against −6.4%, identical
...
| **breakout** | Donchian-55 break, ML confidence gate | **chandelier ATR(3) trailing stop** + long-trend(100) alignment filter | bounded by the trail | 3× × 1/4 |
...
| **vol-prem** | always short variance, 18 Cboe underlyings at equal risk | **two** gates ANDed — the shared VIX term structure, and each sleeve's own curve | one roll | 3× × 1/4, and sized on the tail it would carry ungated (−75.4% day) rather than on its Sharpe |
| **x-sect** | risk-adjusted momentum, top/bottom 30% crypto · 10% equity | none — re-struck at each rebalance | 21 bars (monthly cadence, chosen to keep turnover and cost low) | dollar-neutral; 3× × 1/4 |
| **BAB** | beta-neutral long low-β / short high-β, top 20% of the top-100 liquid names | none — re-struck at each rebalance | 21 days | beta-neutral by Frazzini-Pedersen leg scaling; 3× × 1/4 |
...
| **global-macro** | the same TSMOM tranches on EM FX and commodities | signal flip | to the flip | 3× × 1/4 |
...
- **Probability of backtest overfitting (CSCV, §6, `scripts/run_cscv.py`):** across all
  ...
  **+0.088 to +0.003 Sharpe/bar** out of sample (P(selected loses OOS) = 46%) — a quantified confirmation that
...
- **A final out-of-sample block, held to the end and run exactly once** (§10/§11) — `OOS_START=2024-07-01`,
  ...
  so it is the deliverable's scorecard (Sharpe **3.80**, 5/5); the 15-year window is reported
...
- **A rolling & anchored walk-forward with periodic re-fitting** (§10) at the portfolio level
  ...
  track over ~20 years (2013→2026), Sharpe 3.86, max-DD −10.5%** — the book is out-of-sample across nearly the
  ...
  annual re-fit all land Sharpe in **[+3.85, +3.86]** (spread 0.01). **Crisis-window
  stress** on this long track: through the **2008 GFC** the book draws down only **not covered** on the
  no legs live then (the crisis / managed-futures leg hedges the volprem short-vol tail),
  −5.3% through 2018 Volmageddon and −8.2% through COVID. *(Caveat: the pre-2020
...
months the standard error of the OOS Sharpe is ±0.70, so 3.80 is 3.80 ±0.70**, and the block is a benign
...
| **Breakout** | ungated Donchian-55 **+0.41**, DD −13.3%, prec 36% | LightGBM gate **+1.01**, DD **−2.7%**, prec 39% (OOS **0.20→0.47**) — purged **k-fold**, whose earliest fold trains 100% on its own future; under an expanding **walk-forward** on net-of-cost labels the same gate is **OOS 0.20→0.64** ([BREAKOUT §13](docs/strategies/BREAKOUT.md#13-the-ml-gate-under-a-walk-forward)) | **Risk reduction + OOS robustness** — and the one family where the gate also **lifts OOS**: filtering false breakouts pays most in chop and rescues the 1h timeframe. The lift survives the walk-forward at a lower level, so it is a haircut on the estimate, not a refutation. |
...
one with the least room — the **worst month**, at −6.4% against −6% — while adding something.
...
|---|---|---|---|---|---|---|
| **baseline (shipped)** | **+3.99 / +3.76** | **51.8% / 34.7%** | **−9.1%** | **−5.3%** | **77%** | **3** |
| breakout raw (no ML) | +4.10 / +4.04 | 54.4% / 39.5% | −9.1% | −5.3% | 80% | 2 |
| breakout + ML *(shipped)* | +4.07 / +3.75 | 53.9% / 36.7% | −9.1% | −5.3% | 77% | 3 |
| trend raw | +2.98 / +3.10 | 32.8% / 34.6% | −8.6% | −4.3% | 77% | 3 |
| trend + LightGBM gate | +3.22 / +3.41 | 29.5% / 31.3% | −6.8% | −5.5% | 78% | 5 |
| trend + RF gate | +3.26 / +3.65 | 28.0% / 31.4% | −6.8% | −5.5% | 81% | 3 |
| carry + timing overlay | +3.99 / +3.76 | 51.8% / 34.7% | −9.1% | −5.3% | 77% | 3 |
...
book, it moves *one* leg out of a regime the other three still trade through, so the flat days are not flat
months.
...
gate to all four legs a-priori, hard-gating **loses** OOS Sharpe against the ungated
3.80 either way (logistic 3.57, boosting 3.83) — and it
**cuts OOS months-in-profit to 73%–77%** (from 84.6%) and returns the
full-window **streak to 4**; it trades away the exact metrics that bind, so any Sharpe bump bought that way is
not an improvement. The cherry-pick (gate only where it helps standalone) reads
3.99 full-sample but breaks
...
**77.3% → 59–61%**, Sharpe **3.99 → 3.15–3.30**, CAGR
**52% → 31–35%** on the full window (compounding **441× →
50–83×** the starting capital), taking the book from **5/5 to 3/5**; the marginal OOS uptick is short-block
OOS-fit, and a **constant** cut to the same average exposure matches it (a 20-draw **random** gate spans 3.25–3.77
...
crash months while the other three families stay invested and earning. Reproduce: `make ml-portfolio`
(`scripts/run_ml_portfolio_overlay.py` → `reports/book/ml_portfolio_overlay.json`).

## 6. Ceiling assessment & honest limits

- **Reachable here:** a diversified four-family book at full-sample Sharpe ≈ **3.94** net
  (+47.4%/yr on the brief's $500k at 1.15×, drawdown −9.4%,
  block-bootstrap MC-P5 **+3.31**) that meets **all five** targets on the window the brief
  scores — the final out-of-sample block (Sharpe **3.80**) — and **two of the five** on the full
  15-year window (Sharpe **3.94**, missing on months-in-profit under 80% and a 3-month losing streak against ≤2 and a worst month past −6%). Vol-premium anchors the Sharpe;
  three decorrelated sources cut its tail; a VIX-term-structure regime gate times the
  ...
  what binds now**, at −6.4% against −6%, and the honest reading is that the five together are at
...
- **Reweighting cannot close months-in-profit — but tail-timing can.** Every *static reweighting* route
  ...
  *under*-weights the short-vol leg only when the curve inverts — *avoiding* the crashes rather than trading them —
  ...
  once (unlevered A/B; worst month **−6.4%** at the shipped 1.15× of §4b). That the overlay beats
...
- **The *reweighting* frontier, quantified** — it is the *weighting* axis that is capped; the VIX tail-timing
  ...
  **78.4%** *and* worst-month **−6.4%** together, which the weighting axis cannot do at
  any weight. That beats the frontier: the scorecard reads **5 of 5** on the scored block and
  **2 of 5** on the full window. The mechanism is the right one, and on its own it was still not
  ...
  the core-family book; the deliverable is the four-family master with the VIX overlay.)*
- **The crisis-alpha leg is still worth holding, but not at an earner's weight (§6c-ter).** Dropping it
  outright *raises* Sharpe to **+3.96** and CAGR to **+60.2%** — but that
  overshoots the ≤4.0 band and takes the worst month to **−6.30%**, so the book buys
...
- **Binding constraints:** costs/turnover kill 5m (and most 15m) sleeves; the surviving edge is
  **crypto-heavy** (no family spans both classes since trend was dropped), so the book carries crypto-regime risk (visible as the
  book's thinnest years — 2022 at +0.5 — positive but well below the
...
- **Short-vol / variance risk premium** (Cboe VIX/VXN/RVX/… vs realised, [docs/strategies/VOLPREM.md](docs/strategies/VOLPREM.md)) —
  ...
  **18-underlying Cboe book** (equity-index / single-name / international / commodity incl. gold-miners /
  ...
  but Sharpe overstates for the UNGATED construction — skew **−18.4** and a
  **−75.4%** systemic-vol day, which the two gates take to −3.3 and
  −13.6% on the shipped series — a tail that
  ...
  its tail dominates. In the master book it sits at equal weight (1/4) and anchors the Sharpe; must be
...
**four of them are traded** — short-vol / VRP, cross-sectional momentum, trend and breakout; §6d-ter prices what leaving the rest out costs — and
those four are what §4 assembles. Cross-sectional **reversal**, stat-arb **pairs**, the
...
|---|---|---|
| 0 (shipped) | +4.01 / +63.8% / **−6.3%** / 76% | **+3.76** / +41% |
| 0.15 | +4.08 / +63.0% / −5.8% / 77% | +3.02 / +48% |
| 0.25 | +4.07 / +62.3% / −5.1% / 77% | +2.36 / +52% |
| 0.40 | +3.96 / +60.6% / −4.1% / 75% | +1.80 / +56% |
...
worst day to −13.6%. This section prices a hedge for it anyway, for the reason a gate does
...
standalone Sharpe **+0.16** over 21 years,
**+1.2%** a year, **-0.34** on the frozen block. Two questions had to be
...
|---|---|---|---|
| equity | +0.23 | +0.14 | 34x |
| commod | +0.59 | +0.50 | 29x |
| bond | -0.01 | -0.29 | 91x |
| fx | +0.11 | -0.18 | 70x |
| crypto | +0.87 | +0.84 | 4x |
...
+0.50 → +0.39: sectors are the S&P again.

| | standalone Sharpe | 2008 GFC | 2011 selloff | 2018 Q4 | COVID crash | yen unwind 2024 |
|---|---|---|---|---|---|---|
| sign blend (ships) | +0.16 | **+33.2%** | **+12.7%** | **+4.3%** | **+11.9%** | **+2.4%** |
| EWMAC + response function | **+0.50** | +23.3% | −4.4% | +3.9% | +11.2% | +0.6% |

That is the whole trap in one table. On Sharpe alone EWMAC is the upgrade — **+0.50
against +0.16**, +6.5% a year against +1.2% — and it turns
COVID from +11.9% into +11.2%. A leg selected on its own ratio would have
...
book's losing streak to **3 months** and months-in-profit down to 72%,
for +39.0% against +60.2% without it. Dropping it is not the answer either — that
is what takes the worst month to −6.30% and pushes Sharpe past the top of the band.

| the hedge slot | full window: Sharpe / CAGR / worst month / months / streak | frozen block: targets / Sharpe / CAGR |
|---|---|---|
| dropped entirely | +3.96 / +60.2% / −6.30% / 77% / 3 | 5/5 / +3.76 / +40.8% |
| one full slot (was shipped) | +2.98 / +39.0% / −9.90% / 72% / 3 | 4/5 / +2.83 / +29.5% |
| half a slot, constant | +3.44 / +47.6% / −8.50% / 73% / 5 | 4/5 / +3.38 / +35.1% |
| **ramped on market stress** (ships) | +3.47 / +49.0% / **−9.20%** / 71% / 3 | **4/5** / +3.31 / +35.2% |
...
(0.70 of a slot) at the times it pays for it**: 10.0% of CAGR on the full
window and 5.7% on the frozen block, a worst month of −9.20%, and
the streak from 3 to 3.
...
|---|---|---|
| ramped on market stress | **+49.0%** / **−9.20%** | **+35.2%** |
| the same ramp, rotated to the wrong days | +45.0% / −9.00% | +33.9% |
| held flat at the ramp's own average | +44.0% / −8.60% | +32.7% |
...
finally took the full window to 3 months was sizing the long-gamma hedge on market stress
...
## 6d-ter. The shipped composition is four families, and it was chosen against the scorecard (§12)
...
sized on stress, 0 of the 163 configurations clear the full window and so
0 clear both, while eight clear the scored block — the shipped book
...
(months 70.2%, streak 5) and **3/5 on the frozen block** (Sharpe 2.30, months 61.5%).
Running every single- and double-removal — 163 configurations, `make composition`
(`scripts/run_composition_search.py` → `reports/book/composition_search.json`) — gives:

| configuration | full window | frozen block |
|---|---|---|
| all eight | 3/5 — Sharpe 2.53, months 70.2%, streak 5 | 3/5 — Sharpe 2.30 (under 2.5), months 61.5% |
| drop vol-premium | 0/5 — Sharpe 0.64 (under 2.5), months 34.6%, max−DD −20.7%, streak 54, worst month −6.4% | 2/5 — Sharpe 0.79 (under 2.5), months 46.2%, streak 3 |
| drop x-sect | 3/5 — Sharpe 2.50, months 69.7%, streak 5 | 2/5 — Sharpe 1.97 (under 2.5), months 57.7%, streak 3 |
| drop breakout | 3/5 — Sharpe 2.53, months 70.2%, streak 4 | 3/5 — Sharpe 2.46 (under 2.5), months 65.4% |
| drop crisis | 2/5 — Sharpe 3.12, months 72.9%, streak 6, worst month −6.5% | 4/5 — Sharpe 2.91, months 61.5% |
| drop global-macro | 2/5 — Sharpe 2.99, months 70.7%, streak 3, worst month −10.0% | 4/5 — Sharpe 2.82, months 65.4% |
| drop BAB | 3/5 — Sharpe 2.50, months 68.6%, streak 11 | 3/5 — Sharpe 2.39 (under 2.5), months 65.4% |
| drop trend | 1/5 — Sharpe 2.45 (under 2.5), months 67.0%, streak 5, worst month −7.3% | 3/5 — Sharpe 2.28 (under 2.5), months 65.4% |
| drop carry | 2/5 — Sharpe 2.45 (under 2.5), months 70.2%, streak 5 | 3/5 — Sharpe 2.03 (under 2.5), months 69.2% |
| *(154 further pairs)* | — | fail at least one |
...
**Eight of 163 clear the scored block, and that ratio is the point.** A
163-way search that returns eight survivors is weak evidence by construction,
...
composition is now one survivor among eight rather than one of two, which is *weaker*
evidence than the section used to carry, not stronger.

**Neither removed leg is weak on its own terms.** Trend's standalone (0.87) sits between
breakout (0.92) and x-sect (0.83); carry is **1.16**, the second-highest of the eight. What
...
65.4% months-in-profit; dropping carry as well is what lifts the block to
84.6%.
...
middle column is the eight-family book levered to **1.31×**, which puts it on the shipped
book's realised volatility — the only like-for-like comparison:

| | eight families | eight, vol-matched | four families *(shipped)* |
|---|---|---|---|
| targets, full / block | 3/5 · 3/5 | 2/5 · 3/5 | **2/5 · 5/5** |
| Sharpe, full / block | 2.53 / 2.30 | 2.55 / 2.38 | 3.94 / **3.80** |
| **CAGR, full / block** | +29.3% / +19.7% | +33.6% / +23.5% | **+59.4% / +38.1%** |
| **P&L /yr on $500k** | $131k | $148k | **$237k** |
| book volatility | 10.5% | 11.8% | **12.0%** |
| max-DD / worst month | −9.2% / −5.7% | −11.6% / −6.4% | **−9.4% / −6.4%** |
| vol-premium share of P&L | 67% | — | **79%** |

**Returns did not fall — they rose.** At the shipped 1.15× the four-family book earns
**+30.0pp of CAGR more** on the full window and **+18.5pp more** on the scored block,
about **$105k/yr more** on the brief's $500k. The Sharpe fell because the book runs *hotter*: six
legs at equal risk are less diversified than eight, so volatility went from 10.5% to 12.0% and the
...
At **matched risk** the wider book does edge it — the eight-family book at 1.31× earns
−25.7pp more on the full window and −14.6pp more on the block — so the honest
...
3/5 and 3/5 at that leverage, because the targets it misses are
...
67% to 79% of P&L against the tail it would carry ungated, and trend was the only family
...
eight-family book with §6d-bis's frontier is that submission; the four-family book is the one that passes.
...
unchanged — the leg is 82% of P&L, and gating it better does not make the book less
dependent on it.
...
**1. "A net Sharpe near 3.94 is not credible for a real book."** It would not be for a *sleeve*, and no sleeve here
earns it: the best single sleeve's **deflated Sharpe is 0.00 at N=2,129 trials**, and the same
selection walk-forwarded out-of-sample gives **+0.09**. The book's number comes from *not selecting* —
four premia at mean pairwise correlation **0.06**, each applied uniformly across its
whole universe. The check that matters: **remove the anchor leg and the remaining three
still make Sharpe +1.03**, positive every year. If the number were a mining artifact it would
collapse there.

**2. "Half the P&L is one leg with a −78% tail."** Half the P&L, yes; the tail belongs to the ungated
construction. The shipped leg's worst day is −13.6% on a −22.8% drawdown, and
...
is **82% of P&L**. Three things bound it — it is sized on the tail rather than on its Sharpe (equal risk,
...
**±0.70**, stated in §5c — 3.80 is 3.80 ± 0.70. The length is a trade: at one year the ≤2-month streak
target is close to a coin flip, and the crypto legs only list from 2020 so a five-year block leaves one year
to build on. The wider evidence is the book-level walk-forward, which runs out-of-sample **2013→2026 at
Sharpe 3.86** and pays for that history in drawdown (−10.5%).

**4. "It is a crypto book with a hedge bolted on."** Three of four families are crypto-only and
...
is **5 of 5** — missing on nothing — and the full window is **2 of 5**, missing
on months-in-profit under 80% and a 3-month losing streak against ≤2 and a worst month past −6%. Every one of the four *risk* targets clears on both windows; what the long window fails is
...
worst month from −9.90% to −9.20% and the streak from
3 to 3 — the risk got *better* and the ratio walked out of the
...
*down*, from 77.3% to 59–61%. §6c searched for a source that **earns** through such
...
make composition    # §6d-ter: the 163-way composition search behind the four-family book

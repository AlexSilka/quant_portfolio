...
def _monthly_hit_chunk(x: np.ndarray, mlen: int) -> float:
    ...
    if not n_full:
        return np.nan
    m = (1.0 + x[:n_full].reshape(-1, mlen)).prod(axis=1) - 1.0
...
def mc_metrics(net_ret: pd.Series, periods_per_year: float, n_reps: int = 1000,
    ...
    """Block-bootstrap P5/P50/P95 for Sharpe, max-drawdown and monthly hit rate together.

    One resampling scheme (stationary block bootstrap, dependence-preserving) drives all three
    metrics so they are mutually consistent. The monthly hit rate is computed by chunking each
    ...
    sh, dd, hit = np.empty(n_reps), np.empty(n_reps), np.empty(n_reps)
    for i, (pos_args, _) in enumerate(bs.bootstrap(n_reps)):
        x = pos_args[0]
        sd = x.std(ddof=1)
    ...
    return _pack(sh, dd, hit, block_len=round(block, 2), n_reps=n_reps)
...
def trade_order_mc(net_ret: pd.Series, periods_per_year: float, n_reps: int = 1000,
    ...
    for i in range(n_reps):
        ...
        dd[i] = _maxdd(x)
        hit[i] = _monthly_hit_chunk(x, mlen)
    return _pack(sh, dd, hit, n_reps=n_reps)
...
def entry_jitter_mc(net_ret: pd.Series, periods_per_year: float, n_reps: int = 1000,
    ...
    *realised* P&L series a uniform shift only drops |k| endpoints and re-bins month boundaries, so Sharpe
    ...
    return _pack(sh, dd, hit, n_reps=n_reps, max_lag=max_lag)
...
def random_start_mc(net_ret: pd.Series, periods_per_year: float, n_reps: int = 1000,
    ...
    for i in range(n_reps):
        ...
        dd[i] = _maxdd(x.to_numpy())
        hit[i] = _real_monthly_hit(x)
    return _pack(sh, dd, hit, n_reps=n_reps, window_frac=frac)


def mc_all_variants(net_ret: pd.Series, periods_per_year: float, n_reps: int = 1000,
                    seed: int = 7) -> dict:
    """All four Monte-Carlo schemes the brief requires, each with P5/P50/P95 of Sharpe, max-DD and
    monthly hit rate: stationary block bootstrap, trade-order resampling, entry jitter (±1-3 bars) and
    randomised start dates. Returns {variant: {metric_pXX}}."""
    return {
